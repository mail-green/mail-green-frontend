...
export function MailCardGroupList({ cards }: { cards: SenderMailCard[] }) {
        ...
        <div>
            {group1000.length > 0 && (
                <>
                    ...
                    {group1000.map(card => <MailCardItem key={card.name} card={card} />)}
                </>
            )}
            {group500.length > 0 && (
                <>
                    <div className="font-bold text-xl mb-2 mt-4">500개+</div>
                    {group500.map(card => <MailCardItem key={card.name} card={card} />)}
                </>
            )}
            {group100.length > 0 && (
                <>
                    <div className="font-bold text-xl mb-2 mt-4">100개+</div>
                    {group100.map(card => <MailCardItem key={card.name} card={card} />)}
                </>
            )}
            {group50.length > 0 && (
                <>
                    <div className="font-bold text-xl mb-2 mt-4">50개+</div>
                    {group50.map(card => <MailCardItem key={card.name} card={card} />)}
                </>
            )}
            {groupMini.length > 0 && (
                <>
                    <div className="font-bold text-xl mb-2 mt-4">기타</div>
                    {groupMini.map(card => <MiniMailCardItem key={card.name} card={card} />)}
                </>
            )}
        </div>
    );
